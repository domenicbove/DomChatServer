...
class Listener extends Thread {
    ...
    public void run() {    
    	...
    	try {
			...
			if(nextString.contains(":FRIEND")){        //if client sends a friend an invitation
				...
				if(friend == null){
					...
					sendChatInvitation(friend, groupOwner);
				}
			}
		...
		//	members.add(outStream);							//add clients printstream to it
			...
			groupsHashMap.putIfAbsent(groupOwner, new Group()); //if groupOwner doesn't have a group, make him one, put in hashmap
			group = groupsHashMap.get(groupOwner);				//grab the groupOwners group
			group.addUser(u);
			
			membersPrintStream = group.getMembersStream();
	        ...
	        while (true) {
	        	...
	        	if(data.contains(":QUEUE")){  //comes in as ":QUEUE,s1,s2,s3"
	        		ArrayList<String> playlist = new ArrayList<String>();
	        		String[] split = data.split(",");
	        		for(int i=1; i<split.length; i++){
	        			playlist.add(split[i]);
	    			}
	        		group.setPlaylist(playlist);
	        	}
	        	
	        	else if(data.contains(":REQUEST_QUEUE")){
	        		ArrayList<String> playlist = group.getPlaylist();
	        		String playlistMessage = ":QUEUE";
	        		for(String song: playlist){
	        			playlistMessage = playlistMessage +","+song;
	        		}
	        		outStream.println(playlistMessage);
	        	}
	        	
	        	else if(data.contains(":FRIEND")){		//either add next friend or send message out
    ...
    public void sendChatInvitation(User friend, String groupOwner){
